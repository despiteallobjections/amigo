...
type Config struct {
	...
	//
	// It must be safe to call concurrently from multiple goroutines.
	AfterTypeCheck func(info *PackageInfo, files []*syntax.File)
}
...
type PkgSpec struct {
	...
	Files     []*syntax.File // ASTs of already-parsed files
...
type PackageInfo struct {
	...
	Files                 []*syntax.File // syntax trees for the package's files
...
//
func (conf *Config) CreateFromFiles(path string, files ...*syntax.File) {
...
// PathEnclosingInterval returns the PackageInfo and syntax.Node that
// contain source interval [start, end), and all the node's ancestors
// up to the AST root.  It searches all syntax.Files of all packages in prog.
// exact is defined as for astutil.PathEnclosingInterval.
//
// The zero value is returned if not found.
//
/*
func (prog *Program) PathEnclosingInterval(start, end token.Pos) (pkg *PackageInfo, path []syntax.Node, exact bool) {
...
func (conf *Config) Load() (*Program, error) {
	...
	createPkg := func(path, dir string, files []*syntax.File, errs []error) {
	...
	for _, cp := range conf.CreatePkgs {
		files, errs := parseFiles(conf.build(), nil, conf.Cwd, cp.Filenames)
		files = append(files, cp.Files...)

		path := cp.Path
		if path == "" {
			if len(files) > 0 {
				path = files[0].PkgName.Value
			} else {
				path = "(unnamed)"
			}
		}
		...
		if len(files) > 0 && files[0].Pos().IsValid() {
			dir = filepath.Dir(files[0].Pos().RelFilename())
		}
...
//
func (conf *Config) parsePackageFiles(bp *build.Package, which rune) ([]*syntax.File, []error) {
	...
	files, errs := parseFiles(conf.build(), conf.DisplayPath, bp.Dir, filenames)
...
//
func (imp *importer) addFiles(info *PackageInfo, files []*syntax.File, cycleCheck bool) {
...
func (imp *importer) newPackageInfo(path, dir string) *PackageInfo {
	...
	info.checker = types.NewChecker(&tc, pkg, &info.Info)
	imp.progMu.Lock()
