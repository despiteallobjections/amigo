...
func TestLoad_ParseError_AllowErrors(t *testing.T) {
	// syntax.Parse returns (*syntax.File)(nil) for badpkgdecl.go, which
	// loader then strips out of Package.Files, throwing off the test
	// expectations below. Since loader is just a stub for ssa testing,
	// it's not worth fixing this test currently.
	t.Skip("test doesn't work with package syntax")
