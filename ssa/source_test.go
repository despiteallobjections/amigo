...
func TestObjValueLookup(t *testing.T) {
	...
	for _, n := range notes {
		if n.Name != "ssa" {
			t.Errorf("%v: unexpected note type %q, want \"ssa\"", n.Pos, n.Name)
			continue
		}
		if len(n.Args) != 2 {
			t.Errorf("%v: ssa has %d args, want 2", n.Pos, len(n.Args))
			continue
		}
		ident, ok := n.Args[0].(expect.Identifier)
		if !ok {
			t.Errorf("%v: got %v for arg 1, want identifier", n.Pos, n.Args[0])
			continue
		}
		exp, ok := n.Args[1].(string)
		if !ok {
			t.Errorf("%v: got %v for arg 2, want string", n.Pos, n.Args[1])
			continue
		}
		...
		pos := p
		key := fmt.Sprintf("%s:%d", ident, pos.RelLine())
...
// corresponding to every syntax.Expr.
