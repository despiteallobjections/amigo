...
func findEnclosingPackageLevelFunction(pkg *SSAPackage, path []syntax.Node) *Function {
	if n := len(path); n >= 2 { // [... {Gen,Func}Decl File]
		...
		case *syntax.FuncDecl:
				...
				for _, b := range pkg.init.Blocks {
					for _, instr := range b.Instrs {
						if instr, ok := instr.(*Call); ok {
							if callee, ok := instr.Call.Value.(*Function); ok && callee.Pkg == pkg && callee.Pos() == decl.Name.Pos() /*NamePos*/ {
								return callee
							}
						}
					}
			...
			return findNamedFunc(pkg, decl.Name.Pos() /*NamePos*/)
		}
	}
	return nil // not in any function
}
...
//      github.com/mdempsky/amigo/types.Info.Types[e].Value instead.)
...
// astutil.PathEnclosingInterval to locate the syntax.Node, then
...
//
// ref is the path to an syntax.Name (e.g. from PathEnclosingInterval),
