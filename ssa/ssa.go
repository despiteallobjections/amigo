...
//
// Pos() returns the declaring syntax.FuncLit.Type.Func or the position
// of the syntax.FuncDecl.Name, if the function was explicit in the
...
// variable.
//
// Pos() returns the position of the syntax.ValueSpec.Names[*]
// identifier.
//
type Global struct {
	name   string
...
//
// Pos() returns the syntax.CompositeLit.Lbrace for a composite literal,
// or the syntax.CallExpr.Rparen for a call to new() or for a call that
...
//
// Pos() returns the syntax.UnaryExpr.OpPos, if explicit in the source.
...
//
// Pos() returns the syntax.CallExpr.Lparen, if the instruction arose
...
//
// Pos() returns the syntax.CallExpr.Lparen if the instruction arose from
// an explicit T(e) conversion; the syntax.AssertExpr.Lparen if the
// instruction arose from an explicit e.(T) operation; or syntax.NoPos
// otherwise.
//
// Example printed form:
...
//
// Pos() returns the syntax.FuncLit.Type.Func for a function literal
// closure or the syntax.SelectorExpr.Sel for a bound method closure.
//
// Example printed form:
...
//
// Pos() returns the syntax.CallExpr.Lparen, if created by make(map), or
// the syntax.CompositeLit.Lbrack if created by a literal.
//
// Example printed form:
...
//
// Pos() returns the syntax.CallExpr.Lparen for the make(chan) that
// created it.
//
// Example printed form:
// 	t0 = make chan int 0
...
//
// Pos() returns the syntax.CallExpr.Lparen for the make([]T) that
// created it.
//
// Example printed form:
...
//
// Pos() returns the position of the syntax.SelectorExpr.Sel for the
...
// Type() returns a (possibly named) *types.Pointer.
//
// Pos() returns the syntax.IndexExpr.Lbrack for the index operation, if
...
// syntax.CaseClause.Case if the instruction arose from a case of a
...
//
// Pos() returns the syntax.CallExpr.Lparen if this panic was explicit
// in the source.
//
// Example printed form:
// 	panic t0
...
// Value.
//
// Pos() returns the syntax.KeyValueExpr.Colon or syntax.IndexExpr.Lbrack,
...
//
// If Expr is an *syntax.Name denoting a var or func, Object() returns
...
// mode flag.
//
// DebugRefs are not emitted for syntax.Idents referring to constants or
// predeclared identifiers, since they are trivial and numerous.
// Nor are they emitted for syntax.ParenExprs.
//
