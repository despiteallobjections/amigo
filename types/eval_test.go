...
func TestEvalPos(t *testing.T) {
	...
	pkg, err := conf.Check("p", files, nil)
	if err != nil {
		t.Fatal(err)
	}

	// TODO(mdempsky): Enable test.
	_ = pkg
	...
	//					str, val := split(str, "=>")
	//					testEval(t, pkg, comment.Pos(), str, nil, typ, val)
	//				}
	//			}
	//		}
	//	}
...
func f(a int, s string) S {
	...
	// 			if err != nil {
	// 				t.Errorf("%s: %s", pos, err)
	// 				continue
	// 			}
	// 			if obj.String() != wantObj {
	// 				t.Errorf("%s: checkExpr(%s) = %s, want %v", pos, expr, obj, wantObj)
	// 			}
	// 		}
	// 	}
	// }
