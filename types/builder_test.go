	...
	for _, test := range tests {
		...
		if !reflect.DeepEqual(typstrs, test.want) {
			t.Errorf("test 'package %s': got %q, want %q",
				f.PkgName.Value, typstrs, test.want)
		}
	}
}
...
`},
	...
	for _, test := range tests {
		...
		conf.CreateFromFiles(f.PkgName.Value, f)

		lprog, err := conf.Load()
		if err != nil {
			t.Errorf("test 'package %s': Load: %s", f.PkgName.Value, err)
			continue
		}
		...
		if err != nil {
			t.Errorf("test 'package %s': WriteTo: %s", f.PkgName.Value, err)
			continue
		}

		if initbuf.String() != test.want {
			t.Errorf("test 'package %s': got %s, want %s", f.PkgName.Value, initbuf.String(), test.want)
		}
	}
}
