...
func dynimport() {
	...
	if *dynout != "" {
		...
		// TODO(mdempsky): defer f.Flush() and f.Close()?
		out = f
	}

	interp, imports, err := cgo.DynamicImports(*dynobj)
	if err != nil {
		fatalf("%s", err)
	}
	...
	for _, imp := range imports {
		checkImportSymName(imp[0])
		checkImportSymName(imp[1])

		fmt.Fprintf(out, "//go:cgo_import_dynamic %s %s %q\n", imp[0], imp[1], imp[2])
	}
...
func gccgoToSymbol(ppath string) string {
	if gccgoMangler == nil {
		...
		panic("TODO: port pkgpath.ToSymbolFunc to amigo?")
